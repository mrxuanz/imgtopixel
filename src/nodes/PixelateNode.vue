...
const props = defineProps<{ id: string; data: PixelateNodeData; selected?: boolean }>()
...
function update(patch: Partial<PixelateNodeData>) {
...
<template>
  <div class="node" :class="{ selected }">
    ...
    <div class="node-header vue-flow__node-drag-handle">
      <span class="node-icon-wrap pixelate-icon">🎨</span>
      <span class="node-title">{{ t('nodes.pixelate') }}</span>
    ...
    <div class="node-body">
      <div class="param-row">
        <label class="param-label">{{ t('pixelate.mode') }}</label>
        <select :value="data.mode" @change="update({ mode: ($event.target as HTMLSelectElement).value as any })" class="param-select">
          <option value="auto">{{ t('pixelate.modeAuto') }}</option>
          <option value="manual-size">{{ t('pixelate.modeManualSize') }}</option>
          <option value="manual-target">{{ t('pixelate.modeManualTarget') }}</option>
        </select>
      </div>

      <div v-if="data.mode === 'manual-size'" class="param-row">
        <label class="param-label">{{ t('pixelate.pixelSize') }}</label>
        <input type="range" min="1" max="32" :value="data.pixelSize"
          @input="update({ pixelSize: +($event.target as HTMLInputElement).value })"
          class="param-slider" />
        <span class="param-value">{{ data.pixelSize }}</span>
      </div>

      <div v-if="data.mode === 'manual-target'" class="param-row">
        <label class="param-label">{{ t('pixelate.targetW') }}</label>
        <input type="number" min="1" max="512" :value="data.targetW"
          @change="update({ targetW: +($event.target as HTMLInputElement).value })"
          class="param-number" />
        <label class="param-label">{{ t('pixelate.targetH') }}</label>
        <input type="number" min="1" max="512" :value="data.targetH"
          @change="update({ targetH: +($event.target as HTMLInputElement).value })"
          class="param-number" />
      </div>
    </div>
...
.pixelate-icon { background: rgba(99,200,130,0.12); }
...
.node-body { padding: 10px 12px; display: flex; flex-direction: column; gap: 7px; }
.param-row { display: flex; align-items: center; gap: 6px; }
.param-label { color: #888; white-space: nowrap; min-width: 52px; }
.param-slider { flex: 1; accent-color: #637dff; }
.param-select {
  flex: 1; font-size: 12px;
  border: 1px solid #e8e8ef; border-radius: 5px;
  padding: 3px 6px; background: #fafafa; color: #333;
  outline: none;
}
.param-select:focus { border-color: #637dff; }
.param-number {
  width: 52px; font-size: 12px;
  border: 1px solid #e8e8ef; border-radius: 5px;
  padding: 3px 6px; background: #fafafa; color: #333;
  outline: none;
}
.param-number:focus { border-color: #637dff; }
