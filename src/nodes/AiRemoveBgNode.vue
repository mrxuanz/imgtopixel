...
const props = defineProps<{ id: string; data: AiRemoveBgNodeData; selected?: boolean }>()
...
<template>
  <div class="node" :class="{ selected }">
    ...
    <div class="node-header vue-flow__node-drag-handle">
      <span class="node-icon-wrap airemovebg-icon">🤖</span>
      <span class="node-title">{{ t('nodes.aiRemoveBg') }}</span>
      <span v-if="isRunning" class="running-dot" />
      <button v-if="selected && !isRunning" class="node-del-btn" @click.stop="removeNodes([id])">×</button>
    </div>

    <div class="node-body">
      <span class="node-hint">{{ t('aiRemoveBg.hint') }}</span>
    </div>
...
.airemovebg-icon { background: rgba(99,125,255,0.12); }
